...
| XR | Secret | Keys |
|----|--------|------|
| `PostgresInstance` | `<name>-conn` | `host` (private FQDN), `port`, `username`, `dbname` |
| `PostgresInstance` | `<name>-admin-password` | `password` — generated by the provider |
| `RedisInstance` | `<name>-conn` | `host` (private hostname), `port` (TLS) |
...
consumer that finds one can trust it. Nothing here ever holds a credential: Postgres uses the
